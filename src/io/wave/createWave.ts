import { FileBuilder } from "../types";

export const createWave: FileBuilder = (audioData) => {
  if (audioData.type !== "monoral") {
    throw new Error("aaaaa");
  }

  const { source, sampleRate } = audioData;
  const filesize = 2 * source.length + 44;
  ...
  for (let i = 0; i < source.length; i++) {
    const signal = unnormalize(source[i]);
